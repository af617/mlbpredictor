...
LEAGUE_AVGS = {
    "4-Seam Fastball": {"x": -3.0, "z": 16.0},
    "Sinker": {"x": -14.0, "z": 9.0},
    "Cutter": {"x": 3.0, "z": 8.0},
    "Slider": {"x": 6.0, "z": 2.0},
    "Sweeper": {"x": 15.0, "z": 1.0},
    "Curveball": {"x": 9.0, "z": -10.0},
    "Knuckle Curve": {"x": 6.0, "z": -12.0},
    "Changeup": {"x": -13.0, "z": 6.0},
    "Split-Finger": {"x": -10.0, "z": 4.0},
    "Slurve": {"x": 10.0, "z": -2.0}
}
...
@st.cache_resource
def load_model():
    with open("batspeedsmodel_pfx_inplay.pkl", "rb") as f:
...
if "last_pitch_type" not in st.session_state:
    st.session_state.last_pitch_type = None
...
pitch_type = st.sidebar.selectbox("Pitch Type", list(LEAGUE_AVGS.keys()))

if st.session_state.last_pitch_type != pitch_type:
    st.session_state.pfx_x = LEAGUE_AVGS[pitch_type]["x"]
    st.session_state.pfx_z = LEAGUE_AVGS[pitch_type]["z"]
    st.session_state.last_pitch_type = pitch_type

with st.sidebar.expander("🛠️ Advanced Stats"):
    pfx_x = st.slider(
        "Horizontal Movement (pfx_x)", -20.0, 20.0,
        key="pfx_x",
        help="Horizontal movement in inches from the catcher's perspective."
    )
    pfx_z = st.slider(
        "Vertical Movement (pfx_z)", -20.0, 25.0,
        key="pfx_z",
        help="Induced vertical movement in inches from the catcher's perspective."
    )
...
release_pos_y = st.sidebar.slider("Release: Y-Position (ft)", 45.0, 60.0, 54.0, 0.1)
...
meatball = int(pitch_type in ["4-Seam Fastball", "Changeup"] and abs(plate_x) < 0.5 and 1.5 <= plate_z <= 3.5)
...
if swing == 1:
    ...
    if sum_remaining > 0:
        probs[1] /= sum_remaining
        probs[2] /= sum_remaining
    else:
